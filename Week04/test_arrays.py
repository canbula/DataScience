...
def test_invalid_parameters():
    ...
    for f in files:
        try:
            eval(f[:-3]).replace_center_with_minus_one(2, 3, 5)
            assert False, "Expected an error when m > n"
        except ValueError:
            pass

        try:
            eval(f[:-3]).replace_center_with_minus_one(0, 5, 3)
...
def test_values_outside_center():
    ...
    for f in files:
        for d in range(1, 10):
            ...
            assert outside_center.min() >= 10 ** (
                        d - 1), f"Values outside the center should be more than {10 ** (d - 1)}"
...
def test_negative_n_or_m():
    ...
    for f in files:
        try:
            eval(f[:-3]).replace_center_with_minus_one(2, -5, 3)
